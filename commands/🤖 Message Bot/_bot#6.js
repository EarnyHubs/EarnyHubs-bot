...
var budgets = AdminPanel.getFieldValue({
  ...
  field_name: "Ads_bots_budget" // field name
})
if (message.includes("-")) {
  Bot.sendMessage(
    "❌ *Send please an amount greater or equal to* " + budgets + " " + cur + ""
  )
  Bot.runCommand("/bot#6")
  return
}
...
var no = Libs.ResourcesLib.anotherChatRes("ads", "global")
no.add(+1)
...
var kol = AdminPanel.getFieldValue({
  ...
  field_name: "Ads_bots_click" // field name
})
//100 X 0.01 = 1
...
var junx = Libs.ResourcesLib.anotherChatRes(
  user.telegramid,
  "global-dram" + no.value()
)
junx.set(0)
var nock = User.getProperty("BOT_name")
var data = {
...
Bot.setProperty("#B" + no.value(), data)
var ads = Libs.ResourcesLib.userRes("ads")
ads.add(+1)
if (bot_status == "Enabled ✅") {
  var h = "Disable 🚫"
  var h1 = "/OnOFF disable #B" + no.value() + ""
} else {
  var h = "Enable ✅"
  var h1 = "/OnOFF enable #B" + no.value() + ""
}
Bot.sendInlineKeyboard(
  [
    [
      { title: "✏️ Edit", command: "/editB #B" + no.value() + "" },
      { title: h, command: h1 }
    ]
  ],
  "*Campaign #B" +
