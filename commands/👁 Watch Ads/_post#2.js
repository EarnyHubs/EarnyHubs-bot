...
var budgets = AdminPanel.getFieldValue({
  ...
  field_name: "Ads_watch_budget" // field name
})
if (message.includes("-")) {
  Bot.sendMessage("❌ *Send please an amount greater or equal to* "+budgets+" " + cur)
...
var no = Libs.ResourcesLib.anotherChatRes("ads", "global4")
no.add(+1)

var bot_link = User.getProperty("BOT_link")
var bot_title = "none"
var bot_description = "none"
...
Bot.setProperty("#W" + no.value(), data)
...
if (bot_status == "Enabled ✅") {
  var h = "Disable 🚫"
  var h1 = "/OnOFF disable #W" + no.value()
} else {
  var h = "Enable ✅"
  var h1 = "/OnOFF enable #W" + no.value()
}
Bot.sendInlineKeyboard(
  [
    [
      { title: "✏️ Edit", command: "/editB #W" + no.value() },
      { title: h, command: h1 }
    ]
  ],
  "*Campaign #W" +
