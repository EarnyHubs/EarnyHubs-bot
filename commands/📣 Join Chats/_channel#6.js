...
var budgets = AdminPanel.getFieldValue({
  ...
  field_name: "Ads_chats_budget" // field name
})
if (message.includes("+-")) {
...
var no = Libs.ResourcesLib.anotherChatRes("ads", "global3")
no.add(+1)
...
var kol = AdminPanel.getFieldValue({
  ...
  field_name: "Ads_chats_click" // field name
})
//100 X 0.01 = 1
...
Bot.setProperty("#J" + no.value(), data)
var ads = Libs.ResourcesLib.userRes("ads")
ads.add(+1)
if (bot_status == "Enabled ✅") {
  var h = "Disable 🚫"
  var h1 = "/OnOFF disable #J" + no.value() + ""
} else {
  var h = "Enable ✅"
  var h1 = "/OnOFF enable #J" + no.value() + ""
}
Bot.sendInlineKeyboard(
  [
    [
      { title: "✏️ Edit", command: "/editB #J" + no.value() + "" },
      { title: h, command: h1 }
    ]
  ],
  "*Campaign #J" +
